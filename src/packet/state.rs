...
/// Satellite frequency flags bitfield
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq, BinRead, BinWrite, Serialize, Deserialize)]
#[brw(little)]
pub struct SatelliteFrequencies(u8);

impl SatelliteFrequencies {
    pub fn raw(&self) -> u8 { self.0 }
    pub fn l1_ca(&self) -> bool { self.0 & (1 << 0) != 0 }
    pub fn l1_c(&self) -> bool { self.0 & (1 << 1) != 0 }
    pub fn l1_p(&self) -> bool { self.0 & (1 << 2) != 0 }
    pub fn l1_m(&self) -> bool { self.0 & (1 << 3) != 0 }
    pub fn l2_c(&self) -> bool { self.0 & (1 << 4) != 0 }
    pub fn l2_p(&self) -> bool { self.0 & (1 << 5) != 0 }
    pub fn l2_m(&self) -> bool { self.0 & (1 << 6) != 0 }
    pub fn l5(&self) -> bool { self.0 & (1 << 7) != 0 }
}

impl From<u8> for SatelliteFrequencies {
    fn from(v: u8) -> Self { Self(v) }
}

/// Individual satellite info within a DetailedSatellites packet
#[derive(Debug, Clone, PartialEq, BinRead, BinWrite, Serialize, Deserialize)]
#[brw(little)]
pub struct SatelliteInfo {
    ...
    pub number: u8,
    pub frequencies: SatelliteFrequencies,
...
/// Detailed satellites packet (Packet ID 31, Variable length) - Read only
#[derive(Debug, Clone, PartialEq, BinRead, BinWrite, Serialize, Deserialize)]
#[brw(little)]
pub struct DetailedSatellites {
...
/// North seeking initialisation status
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum NorthSeekingStatus {
    #[default]
    Complete = 0,
    PositionUnknown = 1,
    SolutionOutOfRange = 2,
    SolutionNonOrthogonal = 3,
    RestartedExcessiveMovement = 4,
    RestartedLatitudeChange = 5,
    RestartedLeverArmChange = 6,
    LatitudeCheckFailed = 7,
}

impl From<u16> for NorthSeekingStatus {
    fn from(v: u16) -> Self {
        match v & 0x0F {
            0 => Self::Complete,
            1 => Self::PositionUnknown,
            2 => Self::SolutionOutOfRange,
            3 => Self::SolutionNonOrthogonal,
            4 => Self::RestartedExcessiveMovement,
            5 => Self::RestartedLatitudeChange,
            6 => Self::RestartedLeverArmChange,
            7 => Self::LatitudeCheckFailed,
            _ => Self::Complete,
        }
    }
}
...
pub struct NorthSeekingInitialisationStatus {
    #[br(map = |x: u16| NorthSeekingStatus::from(x))]
    #[bw(map = |x: &NorthSeekingStatus| *x as u16)]
    pub status: NorthSeekingStatus,
    pub version: u16,
